...
📅 Date: ${new Date().toLocaleString()}
        ...
        try {
            // Using a simple webhook service that forwards to email
            const webhookData = {
                to: 'liamseamus1198@gmail.com',
                subject: '🚨 NEW ROOFING LEAD - Storm Damage Assessment',
                message: emailContent,
                from: 'The Storm Professional Website',
                timestamp: new Date().toISOString()
            };

            // Send to a simple webhook service
            ...
            if (webhookResponse.ok) {
                ...
                console.log('⚠️ Webhook failed, but lead was captured');
            }
            
        } catch (webhookError) {
            console.log('⚠️ Webhook sending failed:', webhookError.message);
        }
...
app.get('/health', (req, res) => {
    res.json({
        ...
        port: PORT,
        message: 'Simple email system is ready!'
    });
});
