...
// Enhanced lead submission with Vercel KV
app.post('/api/leads', async (req, res) => {
    try {
        ...
        try {
            ...
            await kv.zadd('leads:timeline', { score: Date.now(), member: lead.id });
            console.log('✅ Lead saved to KV, ID:', lead.id);
        } catch (kvError) {
            console.log('⚠️ KV save failed:', kvError.message);
...
// Enhanced admin API with Vercel KV
app.get('/api/admin/leads', async (req, res) => {
    try {
        // Get all lead IDs from timeline
        const leadIds = await kv.zrange('leads:timeline', 0, -1, { rev: true });
        ...
        console.log('⚠️ KV query failed:', error.message);
        res.status(500).json({ 
            success: false, 
...
// Enhanced status update with Vercel KV
...
// Test endpoint to check KV and leads
app.get('/api/test-leads', async (req, res) => {
    try {
        ...
        if (!leadIds || leadIds.length === 0) {
            return res.json({
                success: true,
                message: 'KV connected, no leads found',
                count: 0,
                leads: []
            });
        }
